...
export class CharacterCreatorUI {
    ...
    createCharacterPanel(index) {
        ...
        // Initialize portrait display - FIXED SPRITE LOADING
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; height: 100%;">
                ...
                <div>
                    ...
                    <div style="background: white; padding: 15px; border-radius: 8px; border: 2px solid #e0e0e0;">
                        ...
                        <!-- FIXED: Individual sprite display with fallback -->
     ...
     */
    setupFooterControls() {
        // Use the existing start simulation button from the modal
        const startBtn = document.getElementById('start-simulation-button');
        if (startBtn) {
            // Remove old listeners
            startBtn.replaceWith(startBtn.cloneNode(true));
            const newStartBtn = document.getElementById('start-simulation-button');
    ...
    /**
     * FIXED: Update character portrait to show individual sprite with fallback
     */
    updateCharacterPortrait(index, spriteSheet) {
        ...
        const createFallbackImage = () => {
            return 'data:image/svg+xml;base64,' + btoa(`
                <svg width="96" height="96" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <circle cx="48" cy="36" r="16" fill="#ccc"/>
                    <path d="M24 72c24-24 48-24 72 0" stroke="#ccc" stroke-width="4"/>
                    <text x="48" y="84" text-anchor="middle" fill="#999" font-size="8">Character ${index + 1}</text>
                </svg>
            `);
        };
        ...
        if (spriteSheet) {
            ...
            portraitImg.onerror = () => {
                console.warn(`⚠️ Failed to load sprite: ${cleanPath} - using fallback`);
                portraitImg.src = createFallbackImage();
                portraitImg.onerror = null; // Prevent infinite loop
            };
        } else {
            // Use fallback immediately if no sprite path
            portraitImg.src = createFallbackImage();
        }
    }
